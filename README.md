...
| 指标 | FP4 | IQ2_XS |
|------|-----|--------|
| 推理速度 | 1.0-1.25 t/s (decode) | ~0.8-1.0 t/s (decode) |
| GPU VRAM | 12.6 GB (常驻 + 200专家缓存) | ~10 GB (常驻 + SLRU缓存) |
| 系统内存 | ~6 GB (稳定) | ~70 GB (全量CPU专家) |
| 模型权重 | 149 GB (FP4) | ~80 GB (IQ2_XS) |
| 缓存策略 | GPU: LFU | GPU: SLRU |
...
# IQ2_XS 推理 (首次启动自动预量化)
python3 generate.py --ckpt-path /models --config /models/config.json \
    --quant-type iq2xs --interactive --max-new-tokens 100 --temperature 0.6
...
系统支持两种量化格式，通过 `--quant-type` 参数选择：

| 特性 | FP4 | IQ2_XS |
|------|-----|--------|
| 比特率 | 4 bit/weight | 2.3125 bit/weight |
| 模型大小 | ~149 GB | ~80 GB |
| 精度 | 较高 | 较低 (MSE ~0.47) |
| GPU缓存策略 | LFU | SLRU |
| CPU缓存 | 禁用 | 全量加载 |
| SSD加载 | seek/read 直接I/O | mmap + OS页缓存 |
| 首次启动 | 直接推理 | 自动预量化(需等待) |
| 推荐场景 | 显存充足、追求精度 | 内存充足、节省磁盘 |
...
```
FP4 权重 → FP4反量化(float32) → IQ2_XS量化(GPU加速) → .iq2xs归档文件
```

- 优先使用 GPU 加速量化（`quantize_iq2xs_gpu_optimized`）
- GPU 不可用时回退到 CPU（C 实现通过 ctypes 调用）
...
MoE.forward(x, input_ids)
    ...
    │       │
    │       ├─▶ L2 CPU 缓存命中? → H2D 传输 (~5ms)
    │       │
    │       └─▶ L3 SSD 读取? → 直接 I/O + H2D (~30ms)
    │
    ├─▶ 路由预测预取: RoutePredictor → 预取L+2层专家到GPU
    │
...
| 缓存 | `inference/expert_cache.py` | 三级缓存(GPU/CPU/SSD)、SLRU策略、路由预测预取 |
...
| IQ2_XS GPU量化 | `inference/quantize_iq2xs_gpu_optimized.py` | GPU 加速 IQ2_XS 量化 |
| IQ2_XS C封装 | `inference/iq2xs_c_wrapper.py` | Python ctypes 调用 C 量化实现 |
| IQ2_XS 参考 | `inference/iq2xs_gemm.py` | 纯 PyTorch IQ2_XS GEMM 参考实现 |
...
| `csrc/iq2_xs.cu` | IQ2_XS CUDA GPU 实现 |
...
| 缓存层 | 容量 | 策略 | 延迟 |
|--------|------|------|------|
| L1 GPU | 动态计算 | SLRU | 0ms |
| L2 CPU | 全量专家 (~30GB) | 全量加载 | ~5ms (H2D) |
| L3 SSD | 全量专家 | mmap + OS页缓存 | ~30ms |

**关键设计决策**：
- **FP4 L2 CPU 缓存禁用**: pinned memory 会锁定物理页，96GB 内存中模型常驻权重 + OS 已占 ~40GB，剩余空间不足以缓存大量专家
- **IQ2_XS CPU 全量加载**: IQ2_XS 每专家仅 ~7.5MB，全量 11008 专家约 80GB，96GB 内存可容纳
...
- **Protected 段**: 被多次访问的热点专家，不会被轻易淘汰
- **Probation 段**: 新进入缓存的专家，再次访问后晋升到 protected 段
- **淘汰顺序**: 先淘汰 probation 段最久未访问的，再淘汰 protected 段

相比 LFU，SLRU 对访问模式变化更敏感，能更快适应热点转移。
...
```
GPU 计算 L 层 → CPU 预取 L+2 层专家到 GPU → SSD 预取 L+5 层专家到 CPU
```
...
# Python IQ2_XS 推理测试
python3 generate.py --ckpt-path /models --config /models/config.json \
    --quant-type iq2xs --interactive --max-new-tokens 50 --temperature 0.6
```
