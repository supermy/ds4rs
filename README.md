...
| 系统内存 | ~6 GB (稳定) | ~80 GB (全量CPU专家) |
| 模型权重 | 149 GB (FP4) | ~80 GB (IQ2_XS) |
| GPU 缓存策略 | LFU | SLRU (90% protected / 10% probation) |
| GPU 命中率 | ~6% | ~78% (16GB卡) / ~100% (24GB卡) |
| CPU 命中率 | N/A | 100% (全量pinned pool) |
...
| 模型大小 | ~149 GB | ~80 GB |
| 精度 | 较高 | 较低 (MSE ~0.47) |
| GPU缓存策略 | LFU | SLRU (90% protected / 10% probation) |
| GPU命中率 | ~6% | ~78% (16GB) / ~100% (24GB) |
| CPU缓存 | 禁用 | 全量pinned pool (100%命中) |
...
| **CUDA 批量量化 (最终版)** | **23.7 e/s** | 批量+向量化解析 |
