...
#include "ds4_metal.h"
#endif
#ifdef DS4_HAS_TILELANG
#include "ds4_tl_kernels.h"
#endif
#ifdef DS4_HAS_EXPERT_GPU
#include "ds4_expert_gpu.h"
#include "ds4_gpu_weights.h"
#endif
#ifdef DS4_HAS_EXPERT_FP4
#include "ds4_expert_fp4_gpu.h"
#ifdef DS4_HAS_SPARSE_ATTN
#include "ds4_sparse_attn_gpu.h"
#ifdef DS4_HAS_GPU_PIPELINE
#include "ds4_gpu_pipeline.h"
#include "ds4_gpu_kv_cache.h"
#include "ds4_gpu_kernels.h"
#endif
#endif
#endif
...
#ifdef DS4_HAS_EXPERT_GPU
static ds4_expert_gpu_ctx *g_expert_gpu;
#endif
#ifdef DS4_HAS_GPU_WEIGHTS
static ds4_gpu_weights_ctx *g_gpu_weights;
#endif
#ifdef DS4_HAS_EXPERT_FP4
static ds4_expert_official_ctx *g_expert_official;
#endif
#ifdef DS4_HAS_TILELANG
static ds4_tl_kernels *g_tl_kernels;
static ds4_fp8_gemm_ctx g_fp8_gemm;
static int g_fp8_current_layer;
#endif
#ifdef DS4_HAS_SPARSE_ATTN
static ds4_sparse_attn_ctx g_sparse_attn;
#endif
#ifdef DS4_HAS_GPU_PIPELINE
static ds4_gpu_pipeline_ctx g_gpu_pipeline;
static bool g_gpu_pipeline_active;
static ds4_gpu_kv_cache g_gpu_kv_cache;
static bool g_gpu_kv_cache_active;
#else
static void *g_gpu_pipeline = NULL;
static bool g_gpu_pipeline_active = false;
static void *g_gpu_kv_cache = NULL;
static bool g_gpu_kv_cache_active = false;
#endif
...
#ifdef DS4_HAS_TILELANG
    if (g_fp8_gemm.bufs_ready && getenv("DS4_FP8_GEMM") != NULL && g_gpu_weights) {
        void *w_q_a = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wq_a.weight");
        void *w_q_a_s = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wq_a.scale");
        void *w_q_b = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wq_b.weight");
        void *w_q_b_s = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wq_b.scale");
        if (w_q_a && w_q_a_s && w_q_b && w_q_b_s &&
            ds4_fp8_gemm_matvec(&g_fp8_gemm, scratch->qr, norm, 4096, 1024, w_q_a, w_q_a_s, g_tl_kernels->fp8_gemm_q_a)) {
            rms_norm_weight(qr_norm, scratch->qr, q_a_norm, 1024, DS4_RMS_EPS);
            if (ds4_fp8_gemm_matvec(&g_fp8_gemm, q, qr_norm, 1024, 32768, w_q_b, w_q_b_s, g_tl_kernels->fp8_gemm_q_b)) {
                head_rms_norm_inplace(q, DS4_N_HEAD, DS4_N_HEAD_DIM, DS4_RMS_EPS);
                return;
            }
        }
...
#ifdef DS4_HAS_TILELANG
    if (g_fp8_gemm.bufs_ready && getenv("DS4_FP8_GEMM") != NULL && g_gpu_weights) {
        void *w_kv = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wkv.weight");
        void *w_kv_s = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wkv.scale");
        if (w_kv && w_kv_s &&
            ds4_fp8_gemm_matvec(&g_fp8_gemm, scratch->kv_raw, normed, 4096, 512, w_kv, w_kv_s, g_tl_kernels->fp8_gemm_kv)) {
...
#ifdef DS4_HAS_TILELANG
    if (g_fp8_gemm.bufs_ready && getenv("DS4_FP8_GEMM") != NULL && g_gpu_weights) {
        void *w_oa = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wo_a.weight");
        void *w_oa_s = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wo_a.scale");
        void *w_ob = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wo_b.weight");
        void *w_ob_s = ds4_gpu_weights_find_layer(g_gpu_weights, g_fp8_current_layer, "attn.wo_b.scale");
        if (w_oa && w_oa_s && w_ob && w_ob_s) {
            memset(scratch->attn_low, 0, (size_t)n_groups * rank * sizeof(scratch->attn_low[0]));
            bool ok = true;
            for (uint32_t g = 0; g < n_groups && ok; g++) {
                ok = ds4_fp8_gemm_matvec(&g_fp8_gemm, scratch->attn_low + g * rank,
                                     heads + g * group_dim, group_dim, rank,
                                     w_oa, w_oa_s, g_tl_kernels->fp8_gemm_output_a);
            }
            if (ok && ds4_fp8_gemm_matvec(&g_fp8_gemm, out, scratch->attn_low, rank * n_groups, 4096,
                                       w_ob, w_ob_s, g_tl_kernels->fp8_gemm_output_b)) {
                return;
            }
        }
    }
...
#ifdef DS4_HAS_TILELANG
    if (g_fp8_gemm.bufs_ready && getenv("DS4_FP8_GEMM") != NULL && g_gpu_weights) {
        int il = g_fp8_current_layer;
        void *w_gate = ds4_gpu_weights_find_layer(g_gpu_weights, il, "ffn.shared_experts.w1.weight");
        void *w_gate_s = ds4_gpu_weights_find_layer(g_gpu_weights, il, "ffn.shared_experts.w1.scale");
        void *w_up = ds4_gpu_weights_find_layer(g_gpu_weights, il, "ffn.shared_experts.w3.weight");
        void *w_up_s = ds4_gpu_weights_find_layer(g_gpu_weights, il, "ffn.shared_experts.w3.scale");
        void *w_down = ds4_gpu_weights_find_layer(g_gpu_weights, il, "ffn.shared_experts.w2.weight");
        void *w_down_s = ds4_gpu_weights_find_layer(g_gpu_weights, il, "ffn.shared_experts.w2.scale");
        if (w_gate && w_gate_s && w_up && w_up_s && w_down && w_down_s) {
            bool ok = true;
            ok = ok && ds4_fp8_gemm_matvec(&g_fp8_gemm, scratch->shared_gate, x, 4096, 2048, w_gate, w_gate_s, g_tl_kernels->fp8_gemm_se_gate);
            ok = ok && ds4_fp8_gemm_matvec(&g_fp8_gemm, scratch->shared_up, x, 4096, 2048, w_up, w_up_s, g_tl_kernels->fp8_gemm_se_up);
            if (ok) {
                swiglu(scratch->shared_mid, scratch->shared_gate, scratch->shared_up, DS4_N_FF_EXP);
                if (ds4_fp8_gemm_matvec(&g_fp8_gemm, out, scratch->shared_mid, 2048, 4096, w_down, w_down_s, g_tl_kernels->fp8_gemm_se_down)) {
                    return;
                }
            }
        }
...
#ifdef DS4_HAS_EXPERT_GPU
static void layer_routed_moe_one_prealloc_gpu(
        float             * out,
        const ds4_model   * model,
        const ds4_layer_weights * layer,
        const float       * x,
    ...
    ds4_expert_gpu_ctx *gpu = g_expert_gpu;
    const int expert_n_blocks = (int)(expert_in_dim / QK_K);
    const int down_n_blocks = (int)(down_in_dim / QK_K);

    ds4_expert_upload_xq(gpu, xq, expert_n_blocks);

    float *gate_buf = gpu->h_gate_buf;
    float *up_buf   = gpu->h_up_buf;

    for (int i = 0; i < DS4_N_EXPERT_USED; i++) {
        const uint32_t expert = (uint32_t)selected[i];

        uint64_t gate_in_dim, gate_out_dim, gate_row_bytes;
        uint64_t up_in_dim, up_out_dim, up_row_bytes;
        ...
        ds4_expert_iq2_xxs_pair_dot_cached(gpu, gate_buf, up_buf,
                                             gate_base, up_base,
                                             gate_row_bytes, up_row_bytes,
                                             expert_n_blocks, (int)gate_out_dim,
                                             (int)il, (int)expert);

        for (uint64_t j = 0; j < gate_out_dim; j++) {
            float g = gate_buf[j];
            float u = up_buf[j];
            if (clamp > 1.0e-6f) {
                if (g > clamp) g = clamp;
            ...
            mid_all[(uint64_t)i * down_in_dim + j] = silu(g) * u * expert_weight[i];
        }
    ...
    float *down_buf = gpu->h_down_buf;
    for (int i = 0; i < DS4_N_EXPERT_USED; i++) {
        const uint32_t expert = (uint32_t)selected[i];

        uint64_t d_in_dim, d_out_dim, d_row_bytes;
        const uint8_t *down_base = tensor_expert_bytes(model, layer->ffn_down_exps, expert,
                                                        &d_in_dim, &d_out_dim, &d_row_bytes);

        ds4_expert_upload_xq(gpu, midq + (uint64_t)i * down_n_blocks, down_n_blocks);
        ds4_expert_q2_k_dot_cached(gpu, down_buf, down_base,
                                    d_row_bytes, down_n_blocks, (int)d_out_dim,
                                    (int)il, (int)expert);

        for (uint64_t j = 0; j < d_out_dim; j++) {
            out[j] += down_buf[j];
        }
    }
...
static void layer_routed_moe_one_prealloc_dispatch(
        float             * out,
        const ds4_model   * model,
        const ds4_layer_weights * layer,
        const float       * x,
    ...
    if (g_expert_official) {
        ...
        if (il == 0 && getenv("DS4_DEBUG_LOGITS")) {
            fprintf(stderr, "DBG fp4 MoE dims: expert_in_dim=%llu expert_out_dim=%llu down_in_dim=%llu\n",
                    (unsigned long long)expert_in_dim, (unsigned long long)expert_out_dim, (unsigned long long)down_in_dim);
        }
...
#ifdef DS4_HAS_GPU_PIPELINE
        if (g_gpu_pipeline_active) {
            ds4_gpu_pipeline_expert_moe(&g_gpu_pipeline, g_expert_official,
                                         out, x, (int)DS4_N_EMBD, (int)expert_out_dim,
                                         (int)il, selected, expert_weight, DS4_N_EXPERT_USED);
            return;
        }
#endif

        float *gate_buf = (float *)malloc(expert_out_dim * sizeof(float));
        float *up_buf = (float *)malloc(expert_out_dim * sizeof(float));
        float *down_buf = (float *)malloc(DS4_N_EMBD * sizeof(float));

        for (int i = 0; i < DS4_N_EXPERT_USED; i++) {
            const uint32_t expert = (uint32_t)selected[i];
            ds4_expert_official_gate_up(g_expert_official, gate_buf, up_buf,
                                        x, (int)expert_in_dim, (int)il, (int)expert);
            if (getenv("DS4_DEBUG_LOGITS") && (il < 3 || (il >= 18 && il <= 22)) && i == 0) {
                float gmax = 0.0f, umax = 0.0f;
                int gnan = 0, unan = 0;
                for (uint64_t j = 0; j < expert_out_dim; j++) {
                    uint32_t bits; memcpy(&bits, &gate_buf[j], 4);
                    if ((bits & 0x7F800000u) == 0x7F800000u) gnan++;
                    if (gate_buf[j] > gmax || j == 0) gmax = gate_buf[j];
                    memcpy(&bits, &up_buf[j], 4);
                    if ((bits & 0x7F800000u) == 0x7F800000u) unan++;
                    if (up_buf[j] > umax || j == 0) umax = up_buf[j];
                }
                fprintf(stderr, "DBG fp4 L%d E%d gate: max=%.4f nan=%d up: max=%.4f nan=%d\n",
                        il, expert, gmax, gnan, umax, unan);
            }
            for (uint64_t j = 0; j < expert_out_dim; j++) {
                float g = gate_buf[j];
                float u = up_buf[j];
        ...
        for (int i = 0; i < DS4_N_EXPERT_USED; i++) {
            const uint32_t expert = (uint32_t)selected[i];
            float *mid = mid_all + (uint64_t)i * down_in_dim;
            ds4_expert_official_down(g_expert_official, down_buf,
                                mid, (int)down_in_dim, (int)il, (int)expert);
            if (getenv("DS4_DEBUG_LOGITS") && (il >= 14 && il <= 18) && i == 0) {
                float dmax = 0.0f; int dnan = 0;
                for (uint64_t j = 0; j < DS4_N_EMBD; j++) {
                    uint32_t bits; memcpy(&bits, &down_buf[j], 4);
                    if ((bits & 0x7F800000u) == 0x7F800000u) dnan++;
                    if (down_buf[j] > dmax || j == 0) dmax = down_buf[j];
                }
                float mmax = 0.0f; int mnan = 0;
                for (uint64_t j = 0; j < down_in_dim; j++) {
                    uint32_t bits; memcpy(&bits, &mid[j], 4);
                    if ((bits & 0x7F800000u) == 0x7F800000u) mnan++;
                    if (mid[j] > mmax || j == 0) mmax = mid[j];
                }
                fprintf(stderr, "DBG fp4 L%d E%d down: max=%.4f nan=%d mid: max=%.4f nan=%d\n",
                        il, expert, dmax, dnan, mmax, mnan);
            }
            for (uint64_t j = 0; j < DS4_N_EMBD; j++) {
                out[j] += down_buf[j];
            }
...
#ifdef DS4_HAS_EXPERT_GPU
    if (g_expert_gpu) {
        layer_routed_moe_one_prealloc_gpu(out, model, layer, x, il, token, clamp, mid_all, xq, midq);
        return;
    }
#endif
...
static void layer_ffn_one_decode_scratch(
    ...
    t0 = profile ? now_sec() : 0.0;
    layer_routed_moe_one_prealloc_dispatch(scratch->ffn_moe,
                                  model,
                                  layer,
                                  scratch->ffn_norm,
                                  il,
...
static void routed_moe_tokens_worker(void *vctx, uint64_t t0, uint64_t t1) {
    ...
    for (uint64_t t = t0; t < t1; t++) {
        layer_routed_moe_one_prealloc_dispatch(ctx->moe + t * DS4_N_EMBD,
                                      ctx->model,
                                      ctx->layer,
...
static void layer_ffn_shared_batch(
    ...
    if (routed_token_parallel) {
        ...
        for (uint32_t t = 0; t < n_tok; t++) {
            layer_routed_moe_one_prealloc_dispatch(moe + (uint64_t)t * DS4_N_EMBD,
                                          model,
                                          layer,
...
static void layer_forward_raw_swa_one(
    ...
    if (ratio == 4) {
        t0 = profile ? now_sec() : 0.0;
#ifdef DS4_HAS_GPU_PIPELINE
        if (g_gpu_pipeline_active && getenv("DS4_GPU_INDEXER") != NULL) {
            {
                const uint32_t n_head = DS4_N_INDEXER_HEAD;
                const uint32_t ihead_dim = DS4_N_INDEXER_HEAD_DIM;
                float *q = scratch->index_q;
                float *weights = scratch->index_weights;
                matvec_any_decode_scratch(q, model, layer->indexer_attn_q_b, scratch->qr_norm, scratch);
                rope_tail_layer_inplace(q, n_head, ihead_dim, DS4_N_ROT, pos, il, false);
                matvec_any_decode_scratch(weights, model, layer->indexer_proj, scratch->attn_norm, scratch);
                const float iscale = 1.0f / sqrtf((float)(ihead_dim * n_head));
                for (uint32_t h = 0; h < n_head; h++) weights[h] *= iscale;
                comp_allowed = scratch->index_allowed;
                memset(comp_allowed, 0, (size_t)cache->n_index_comp * sizeof(comp_allowed[0]));
                ds4_gpu_indexer_score_from_cpu(&g_gpu_pipeline, comp_allowed,
                                                cache->index_comp_kv,
                                                q, weights,
                                                (int)cache->n_index_comp,
                                                (int)n_head, (int)ihead_dim,
                                                DS4_N_INDEXER_TOP_K < cache->n_index_comp ? DS4_N_INDEXER_TOP_K : (int)cache->n_index_comp);
            }
        } else
#endif
    ...
    t0 = profile ? now_sec() : 0.0;
#ifdef DS4_HAS_GPU_PIPELINE
    if (g_gpu_pipeline_active && getenv("DS4_FULL_GPU_PIPELINE") != NULL) {
        const float *sinks = tensor_data(model, layer->attn_sinks);
        ds4_f32_to_bf16(g_gpu_pipeline.d_q_bf16, scratch->q, DS4_N_HEAD * DS4_N_HEAD_DIM);

        if (g_gpu_kv_cache_active) {
            ds4_gpu_kv_layer *gkvl = &g_gpu_kv_cache.layers[il];
            ds4_f32_to_bf16(g_gpu_pipeline.d_kv_bf16, scratch->kv, DS4_N_HEAD_DIM);
            ds4_gpu_kv_push_raw(gkvl, g_gpu_pipeline.d_kv_bf16);

            if (gkvl->raw_start != 0 && gkvl->n_raw >= gkvl->cap_raw) {
                ds4_gpu_kv_linearize_raw(gkvl, g_gpu_pipeline.d_raw_kv_tmp_bf16);
            }

            ds4_attention_mixed_bf16(g_gpu_pipeline.d_heads_bf16,
                                      g_gpu_pipeline.d_q_bf16,
                                      gkvl->d_raw_kv, gkvl->n_raw,
                                      gkvl->d_attn_comp_kv, gkvl->n_comp,
                                      NULL, sinks,
                                      DS4_N_HEAD, DS4_N_HEAD_DIM);
        } else {
            ds4_f32_to_bf16(g_gpu_pipeline.d_raw_kv_tmp_bf16, cache->raw_kv,
                            (size_t)cache->n_raw * DS4_N_HEAD_DIM);
            ds4_f32_to_bf16(g_gpu_pipeline.d_comp_kv_bf16, cache->attn_comp_kv,
                            (size_t)cache->n_comp * DS4_N_HEAD_DIM);
            ds4_attention_mixed_bf16(g_gpu_pipeline.d_heads_bf16,
                                      g_gpu_pipeline.d_q_bf16,
                                      g_gpu_pipeline.d_raw_kv_tmp_bf16, cache->n_raw,
                                      g_gpu_pipeline.d_comp_kv_bf16, cache->n_comp,
                                      NULL, sinks,
                                      DS4_N_HEAD, DS4_N_HEAD_DIM);
        }
        ds4_bf16_to_f32(scratch->heads, g_gpu_pipeline.d_heads_bf16, DS4_N_HEAD * DS4_N_HEAD_DIM);
    } else
#endif
#ifdef DS4_HAS_SPARSE_ATTN
    if (g_sparse_attn.initialized && getenv("DS4_SPARSE_ATTN") != NULL) {
        const float *sinks = tensor_data(model, layer->attn_sinks);
        ds4_sparse_attn_decode(&g_sparse_attn, scratch->heads,
                                scratch->q,
                                cache->raw_kv, cache->n_raw,
                                cache->attn_comp_kv, cache->n_comp,
                                comp_allowed,
...
static void forward_token_raw_swa_cpu_decode_scratch(
        ...
        ds4_cpu_decode_scratch * scratch) {
#ifdef DS4_HAS_GPU_PIPELINE
    if (g_gpu_pipeline_active && getenv("DS4_GPU_V2_DECODE") != NULL && g_gpu_weights) {
        ds4_embed_hc_from_f16_table(g_gpu_pipeline.d_cur_bf16, g_gpu_pipeline.d_embed_table_bf16,
                                      token, DS4_N_EMBD, DS4_N_HC);
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            ds4_gpu_decode_layer_v2(&g_gpu_pipeline, (int)il, (int)pos, token,
                                     g_gpu_kv_cache_active ? (void *)&g_gpu_kv_cache : NULL,
                                     g_expert_official,
                                     g_gpu_weights,
                                     g_tl_kernels ? (void *)g_tl_kernels->act_quant : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_q_a : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_q_b : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_kv : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_output_a : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_output_b : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_se_gate : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_se_up : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels->fp8_gemm_se_down : NULL,
                                     g_tl_kernels ? (void *)g_tl_kernels : NULL);
            void *tmp = g_gpu_pipeline.d_cur_bf16;
            g_gpu_pipeline.d_cur_bf16 = g_gpu_pipeline.d_next_bf16;
            g_gpu_pipeline.d_next_bf16 = tmp;
        }
        if (logits) {
            void *d_out_w = g_gpu_weights ? ds4_gpu_weights_find(g_gpu_weights, "output.weight") : NULL;
            void *d_out_s = g_gpu_weights ? ds4_gpu_weights_find(g_gpu_weights, "output.scale") : NULL;
            bool gpu_logits_ok = d_out_w && d_out_s && g_tl_kernels && g_tl_kernels->fp8_gemm_output_head;
            if (gpu_logits_ok) {
                ds4_gpu_output_logits_v2(logits, &g_gpu_pipeline,
                                          d_out_w, d_out_s,
                                          (void *)g_tl_kernels->act_quant,
                                          (void *)g_tl_kernels->fp8_gemm_output_head);
            } else {
                float *cur_cpu = scratch->cur;
                ds4_bf16_to_f32(cur_cpu, g_gpu_pipeline.d_cur_bf16, DS4_N_EMBD * DS4_N_HC);
                output_logits_one_decode_scratch(logits, model, weights, cur_cpu, scratch);
            }
            if (getenv("DS4_GPU_COMPARE")) {
                float cpu_logits[DS4_N_VOCAB];
                ...
                for (uint32_t il2 = 0; il2 < DS4_N_LAYER; il2++) {
                    layer_forward_raw_swa_one(next, model, &weights->layer[il2], &cache->layer[il2],
                                              cur, il2, pos, token, scratch);
                    float *tmp2 = cur; cur = next; next = tmp2;
                }
                output_logits_one_decode_scratch(cpu_logits, model, weights, cur, scratch);
                int gpu_argmax = 0; float gpu_max = logits[0];
                int cpu_argmax = 0; float cpu_max = cpu_logits[0];
                for (int i = 1; i < DS4_N_VOCAB; i++) {
                    if (logits[i] > gpu_max) { gpu_max = logits[i]; gpu_argmax = i; }
                    if (cpu_logits[i] > cpu_max) { cpu_max = cpu_logits[i]; cpu_argmax = i; }
                }
                fprintf(stderr, "GPU argmax=%d max=%.4f | CPU argmax=%d max=%.4f\n",
                        gpu_argmax, gpu_max, cpu_argmax, cpu_max);
            }
        }
        return;
    }
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (getenv("DS4_DEBUG_LOGITS")) {
            int nan_count = 0;
            float mx = 0.0f;
            for (uint64_t j = 0; j < (uint64_t)DS4_N_EMBD * DS4_N_HC; j++) {
                uint32_t bits; memcpy(&bits, &cur[j], 4);
                if ((bits & 0x7F800000u) == 0x7F800000u) nan_count++;
                if (j == 0 || cur[j] > mx) mx = cur[j];
            }
            if (nan_count > 0) {
                fprintf(stderr, "DBG decode layer %d: %d NaN/Inf out of %d, max=%.6f\n",
                        il+1, nan_count, DS4_N_EMBD * DS4_N_HC, mx);
            }
        }
    }
        ...
        if (getenv("DS4_DEBUG_LOGITS")) {
            ...
            if (dbg_count < 3) {
                ...
                for (int i = 0; i < DS4_N_VOCAB; i++) {
                    for (int k = 0; k < 5; k++) {
                        if (logits[i] > top5v[k]) {
                            for (int m = 4; m > k; m--) { top5v[m] = top5v[m-1]; top5[m] = top5[m-1]; }
                            top5v[k] = logits[i]; top5[k] = i; break;
                        }
                    }
                }
                fprintf(stderr, "DBG decode[%d] top5:", dbg_count);
                for (int k = 0; k < 5; k++) fprintf(stderr, " %d(%.4f)", top5[k], top5v[k]);
                fprintf(stderr, "\n");
                dbg_count++;
            }
        }
...
static void prefill_layer_major_cpu(
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (getenv("DS4_DEBUG_LOGITS") && (il >= 0 && il <= 5 || il >= 15 && il <= 20)) {
            int nan_count = 0;
            float mx = 0.0f, mn = 0.0f, mean = 0.0f;
            for (uint64_t j = 0; j < (uint64_t)n_tok * hc_dim; j++) {
                uint32_t bits; memcpy(&bits, &cur[j], 4);
                if ((bits & 0x7F800000u) == 0x7F800000u) nan_count++;
                if (cur[j] > mx || j == 0) mx = cur[j];
                if (cur[j] < mn || j == 0) mn = cur[j];
                mean += fabsf(cur[j]);
            }
            mean /= (float)(n_tok * hc_dim);
            fprintf(stderr, "DBG prefill L%d: nan=%d max=%.4f min=%.4f mean_abs=%.4f\n",
                    il+1, nan_count, mx, mn, mean);
        } else if (getenv("DS4_DEBUG_LOGITS")) {
            int nan_count = 0;
            float mx = 0.0f;
            for (uint64_t j = 0; j < (uint64_t)n_tok * hc_dim; j++) {
                uint32_t bits; memcpy(&bits, &cur[j], 4);
            ...
            if (nan_count > 0) {
                fprintf(stderr, "DBG prefill layer %d: %d NaN/Inf out of %llu, max=%.6f\n",
                        il+1, nan_count, (unsigned long long)(n_tok * hc_dim), mx);
            }
        }
    }
    ...
    if (logits) {
        ...
        if (getenv("DS4_DEBUG_LOGITS")) {
            ...
            fprintf(stderr, "DBG prefill top5:");
            for (int k = 0; k < 5; k++) fprintf(stderr, " %d(%.4f)", top5[k], top5v[k]);
            fprintf(stderr, "\n");
        }
    }
...
#ifdef DS4_HAS_TILELANG
    if (g_fp8_gemm.bufs_ready && getenv("DS4_FP8_GEMM") != NULL && g_gpu_weights) {
        void *w_head = ds4_gpu_weights_find(g_gpu_weights, "head.weight");
        int head_dtype = -1;
        for (int i = 0; i < g_gpu_weights->n_tensors; i++) {
            if (strcmp(g_gpu_weights->keys[i], "head.weight") == 0) {
                head_dtype = g_gpu_weights->tensors[i].dtype;
                break;
            }
        }
        if (w_head && head_dtype == DS4_DTYPE_FP8_E4M3) {
            void *w_head_s = ds4_gpu_weights_find(g_gpu_weights, "head.weight.scale");
            if (!w_head_s) w_head_s = ds4_gpu_weights_find(g_gpu_weights, "head.scale");
            if (w_head_s &&
                ds4_fp8_gemm_matvec(&g_fp8_gemm, logits, scratch->output_norm, 4096, 129280, w_head, w_head_s, g_tl_kernels->fp8_gemm_output_head)) {
                return;
            }
        }
    }
...
static void metal_graph_trace_layer_stages(
    ...
    layer_routed_moe_one_prealloc_dispatch(cpu_routed,
                                  model,
                                  layer,
                                  cpu_ffn_norm,
                                  il,
...
struct ds4_engine {
    ...
    bool mtp_ready;
#ifdef DS4_HAS_TILELANG
    ds4_tl_kernels tl_kernels;
    bool tl_kernels_loaded;
#endif
#ifdef DS4_HAS_EXPERT_GPU
    ds4_expert_gpu_ctx expert_gpu;
    bool expert_gpu_loaded;
#endif
#ifdef DS4_HAS_GPU_WEIGHTS
    ds4_gpu_weights_ctx gpu_weights;
    bool gpu_weights_loaded;
#endif
#ifdef DS4_HAS_EXPERT_FP4
    ds4_expert_official_ctx expert_official;
    bool expert_official_loaded;
#endif
};
...
const char *ds4_backend_name(ds4_backend backend) {
    if (backend == DS4_BACKEND_METAL) return "metal";
    if (backend == DS4_BACKEND_TILELANG) return "tilelang";
    return "cpu";
}
...
#ifdef DS4_HAS_TILELANG
    if (e->backend == DS4_BACKEND_TILELANG) {
        e->tl_kernels_loaded = ds4_tl_kernels_load(&e->tl_kernels, "tilelang");
        if (!e->tl_kernels_loaded) {
            fprintf(stderr, "ds4: TileLang GPU kernels not found, using CPU fallback\n");
        }
    }
#endif
#ifdef DS4_HAS_EXPERT_GPU
    if (e->backend == DS4_BACKEND_TILELANG) {
        const char *fp4_dir = getenv("DS4_FP4_MODEL_DIR");
        bool skip_expert_gpu = (fp4_dir && fp4_dir[0]);
        if (!skip_expert_gpu) {
            int cache_slots = DS4_EXPERT_GPU_DEFAULT_SLOTS;
            const char *cache_env = getenv("DS4_EXPERT_CACHE_SLOTS");
            if (cache_env) cache_slots = atoi(cache_env);
            e->expert_gpu_loaded = ds4_expert_gpu_init(&e->expert_gpu, cache_slots);
            if (!e->expert_gpu_loaded) {
                fprintf(stderr, "ds4: Expert GPU init failed, using CPU fallback for experts\n");
            }
            g_expert_gpu = (e->expert_gpu_loaded && getenv("DS4_NO_EXPERT_GPU") == NULL)
                            ? &e->expert_gpu : NULL;
            if (g_expert_gpu) fprintf(stderr, "ds4: expert GPU acceleration enabled (cache=%d slots)\n", cache_slots);
            else fprintf(stderr, "ds4: expert GPU acceleration disabled\n");

        if (g_expert_gpu && getenv("DS4_EXPERT_PRELOAD") != NULL) {
            fprintf(stderr, "ds4: preloading expert weights to GPU cache...\n");
            int preloaded = 0;
            int experts_per_layer = g_expert_gpu->n_cache_slots / DS4_N_LAYER;
            if (experts_per_layer > 256) experts_per_layer = 256;
            if (experts_per_layer < 1) experts_per_layer = 1;
            for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
                const ds4_layer_weights *lw = &e->weights.layer[il];
                for (int ei = 0; ei < experts_per_layer; ei++) {
                    if (preloaded >= g_expert_gpu->n_cache_slots) goto preload_done;
                    uint64_t g_in, g_out, g_rb;
                    const uint8_t *gate = tensor_expert_bytes(&e->model, lw->ffn_gate_exps, ei, &g_in, &g_out, &g_rb);
                    uint64_t u_in, u_out, u_rb;
                    const uint8_t *up = tensor_expert_bytes(&e->model, lw->ffn_up_exps, ei, &u_in, &u_out, &u_rb);
                    uint64_t d_in, d_out, d_rb;
                    const uint8_t *down = tensor_expert_bytes(&e->model, lw->ffn_down_exps, ei, &d_in, &d_out, &d_rb);
                    ds4_expert_iq2_xxs_pair_dot_cached(g_expert_gpu, NULL, NULL,
                                                         gate, up, g_rb, u_rb,
                                                         (int)(g_in / 256), (int)g_out,
                                                         (int)il, ei);
                    ds4_expert_q2_k_dot_cached(g_expert_gpu, NULL, down,
                                                d_rb, (int)(d_in / 256), (int)d_out,
                                                (int)il, ei);
                    preloaded++;
                }
            }
            preload_done:
            fprintf(stderr, "ds4: preloaded %d experts to GPU cache (%d per layer)\n", preloaded, experts_per_layer);
        }
        } else {
            fprintf(stderr, "ds4: skipping expert GPU init (FP4 GEMM path will be used)\n");
            g_expert_gpu = NULL;
        }
    }
#endif
#ifdef DS4_HAS_GPU_WEIGHTS
    if (e->backend == DS4_BACKEND_TILELANG) {
        const char *fp8_path = getenv("DS4_FP8_WEIGHTS");
        if (fp8_path && fp8_path[0]) {
            struct stat fp8_st;
            if (stat(fp8_path, &fp8_st) == 0 && S_ISDIR(fp8_st.st_mode)) {
                e->gpu_weights_loaded = ds4_gpu_weights_load_dir(&e->gpu_weights, fp8_path);
            } else {
                e->gpu_weights_loaded = ds4_gpu_weights_load(&e->gpu_weights, fp8_path);
            }
            if (e->gpu_weights_loaded) {
                ds4_gpu_weights_summary(&e->gpu_weights);
            } else {
                fprintf(stderr, "ds4: FP8 weight loading failed, continuing without GPU weights\n");
            }
        }
        g_gpu_weights = e->gpu_weights_loaded ? &e->gpu_weights : NULL;
    }
#endif
#ifdef DS4_HAS_EXPERT_FP4
    if (e->backend == DS4_BACKEND_TILELANG) {
        const char *fp4_model_dir = getenv("DS4_FP4_MODEL_DIR");
        if (fp4_model_dir && fp4_model_dir[0]) {
            const char *tl_dir = getenv("DS4_TL_DIR");
            if (!tl_dir || !tl_dir[0]) tl_dir = "tilelang";
            fprintf(stderr, "ds4: initializing expert FP4 GEMM module from %s (tl_dir=%s)...\n", fp4_model_dir, tl_dir);
            e->expert_official_loaded = ds4_expert_official_init(
                &e->expert_official, fp4_model_dir, tl_dir);
            if (!e->expert_official_loaded) {
                fprintf(stderr, "ds4: Expert FP4 GEMM init failed, using CPU/IQ2 fallback\n");
            }
        }
        g_expert_official = e->expert_official_loaded ? &e->expert_official : NULL;
        if (g_expert_official) {
            if (e->expert_gpu_loaded) {
                ds4_expert_gpu_cleanup(&e->expert_gpu);
                e->expert_gpu_loaded = false;
            }
            g_expert_gpu = NULL;
            fprintf(stderr, "ds4: expert FP4 GEMM GPU path enabled\n");
        }
    }
#endif
#ifdef DS4_HAS_TILELANG
    if (e->backend == DS4_BACKEND_TILELANG && e->tl_kernels_loaded) {
        g_tl_kernels = &e->tl_kernels;
        if (g_gpu_weights && g_tl_kernels->act_quant && g_tl_kernels->fp8_gemm_q_b) {
            ds4_fp8_gemm_init(&g_fp8_gemm, g_tl_kernels->act_quant);
        }
#ifdef DS4_HAS_SPARSE_ATTN
        if (g_tl_kernels->sparse_attn) {
            ds4_sparse_attn_init(&g_sparse_attn, DS4_N_LAYER, (void *)g_tl_kernels->sparse_attn);
        }
#endif
#ifdef DS4_HAS_GPU_PIPELINE
        if (getenv("DS4_GPU_PIPELINE") != NULL) {
            g_gpu_pipeline_active = ds4_gpu_pipeline_init(&g_gpu_pipeline);
            if (g_gpu_pipeline_active) {
                fprintf(stderr, "ds4: GPU pipeline active (batched expert MoE)\n");
                const char *model_dir = getenv("DS4_FP4_MODEL_DIR");
                if (model_dir && model_dir[0] && getenv("DS4_GPU_INDEXER") != NULL) {
                    ds4_gpu_load_indexer_comp_weights(&g_gpu_pipeline, model_dir);
                }
                if (getenv("DS4_FULL_GPU_PIPELINE") != NULL) {
                    g_gpu_kv_cache_active = ds4_gpu_kv_cache_init(&g_gpu_kv_cache, 128);
                    if (g_gpu_kv_cache_active) {
                        fprintf(stderr, "ds4: GPU KV cache active\n");
                    }
                }
                if (getenv("DS4_GPU_V2_DECODE") != NULL) {
                    ds4_gpu_upload_embed_table_data(&g_gpu_pipeline,
                        tensor_data(&e->model, e->weights.token_embd),
                        e->weights.token_embd->type,
                        e->weights.token_embd->elements);
                    ds4_gpu_upload_output_weights_data(&g_gpu_pipeline,
                        tensor_data(&e->model, e->weights.output_hc_fn),
                        e->weights.output_hc_fn->type,
                        e->weights.output_hc_fn->elements,
                        tensor_data(&e->model, e->weights.output_hc_scale),
                        e->weights.output_hc_scale->elements,
                        tensor_data(&e->model, e->weights.output_hc_base),
                        e->weights.output_hc_base->elements,
                        tensor_data(&e->model, e->weights.output_norm),
                        e->weights.output_norm->type,
                        e->weights.output_norm->elements);
                    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
                        const ds4_layer_weights *lw = &e->weights.layer[il];
                        ds4_gpu_upload_layer_weights_data(&g_gpu_pipeline, (int)il,
                            tensor_data(&e->model, lw->hc_attn_fn), lw->hc_attn_fn->type, lw->hc_attn_fn->elements,
                            tensor_data(&e->model, lw->hc_attn_scale), lw->hc_attn_scale->elements,
                            tensor_data(&e->model, lw->hc_attn_base), lw->hc_attn_base->elements,
                            tensor_data(&e->model, lw->attn_norm), lw->attn_norm->type, lw->attn_norm->elements,
                            tensor_data(&e->model, lw->attn_q_a_norm), lw->attn_q_a_norm->type, lw->attn_q_a_norm->elements,
                            tensor_data(&e->model, lw->attn_kv_a_norm), lw->attn_kv_a_norm->type, lw->attn_kv_a_norm->elements,
                            tensor_data(&e->model, lw->attn_sinks), lw->attn_sinks->elements,
                            tensor_data(&e->model, lw->hc_ffn_fn), lw->hc_ffn_fn->type, lw->hc_ffn_fn->elements,
                            tensor_data(&e->model, lw->hc_ffn_scale), lw->hc_ffn_scale->elements,
                            tensor_data(&e->model, lw->hc_ffn_base), lw->hc_ffn_base->elements,
                            tensor_data(&e->model, lw->ffn_norm), lw->ffn_norm->type, lw->ffn_norm->elements,
                            tensor_data(&e->model, lw->ffn_gate_inp), lw->ffn_gate_inp->type, lw->ffn_gate_inp->elements,
                            lw->ffn_exp_probs_b ? tensor_data(&e->model, lw->ffn_exp_probs_b) : NULL,
                            lw->ffn_exp_probs_b ? lw->ffn_exp_probs_b->elements : 0,
                            lw->ffn_gate_tid2eid ? tensor_data(&e->model, lw->ffn_gate_tid2eid) : NULL,
                            lw->ffn_gate_tid2eid ? lw->ffn_gate_tid2eid->dim[1] : 0,
                            lw->attn_compressor_kv ? tensor_data(&e->model, lw->attn_compressor_kv) : NULL,
                            lw->attn_compressor_kv ? lw->attn_compressor_kv->type : 0,
                            lw->attn_compressor_kv ? lw->attn_compressor_kv->elements : 0,
                            lw->attn_compressor_gate ? tensor_data(&e->model, lw->attn_compressor_gate) : NULL,
                            lw->attn_compressor_gate ? lw->attn_compressor_gate->type : 0,
                            lw->attn_compressor_gate ? lw->attn_compressor_gate->elements : 0,
                            lw->attn_compressor_ape ? tensor_data(&e->model, lw->attn_compressor_ape) : NULL,
                            lw->attn_compressor_ape ? lw->attn_compressor_ape->type : 0,
                            lw->attn_compressor_ape ? lw->attn_compressor_ape->elements : 0,
                            lw->attn_compressor_norm ? tensor_data(&e->model, lw->attn_compressor_norm) : NULL,
                            lw->attn_compressor_norm ? lw->attn_compressor_norm->type : 0,
                            lw->attn_compressor_norm ? lw->attn_compressor_norm->elements : 0);
                    }
                    fprintf(stderr, "ds4: GPU v2 decode pipeline ready (all weights uploaded)\n");
                }
            }
        }
#endif
...
void ds4_engine_close(ds4_engine *e) {
    if (!e) return;
#ifdef DS4_HAS_TILELANG
    if (e->tl_kernels_loaded) ds4_tl_kernels_unload(&e->tl_kernels);
#endif
#ifdef DS4_HAS_EXPERT_GPU
    if (e->expert_gpu_loaded) {
        ds4_expert_cache_stats(&e->expert_gpu);
        ds4_expert_gpu_cleanup(&e->expert_gpu);
    }
#endif
#ifdef DS4_HAS_GPU_WEIGHTS
    if (e->gpu_weights_loaded) {
        ds4_gpu_weights_summary(&e->gpu_weights);
        ds4_gpu_weights_cleanup(&e->gpu_weights);
    }
#endif
#ifdef DS4_HAS_EXPERT_FP4
    if (e->expert_official_loaded) {
        ds4_expert_official_cleanup(&e->expert_official);
    }
#endif
#ifdef DS4_HAS_GPU_PIPELINE
    if (g_gpu_pipeline_active) {
        ds4_gpu_pipeline_cleanup(&g_gpu_pipeline);
        g_gpu_pipeline_active = false;
    }
    if (g_gpu_kv_cache_active) {
        ds4_gpu_kv_cache_cleanup(&g_gpu_kv_cache);
        g_gpu_kv_cache_active = false;
    }
#endif
