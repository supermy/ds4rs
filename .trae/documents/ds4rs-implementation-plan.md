...
| compress_ratios | [0,0,4,128,4,128,...,4,0] | 每层压缩比（44 值：43 层 + 1 MTP 层；层0-1纯SWA, 层2-42交替 ratio=4(有Indexer)/128(无Indexer), 层43(MTP)纯SWA） |
...
| Attention | wo_a | [8, 1024, 4096] (BF16, convert.py 反量化) | 8192 | 4096 | BF16 (原 FP8, convert.py 反量化+reshape) |
...
| 类型 | (N, K) 对 |
|---|---|
| FP8 GEMM | (1024,4096), (32768,1024), (512,4096), (4096,8192), (8192,1024), (2048,4096), (4096,2048) — 注意 (8192,4096) wo_a 已转为 BF16，不再走 FP8 GEMM |
...
- **wo_a 特殊处理**：convert.py 将 wo_a 从 FP8 反量化为 bfloat16 并融合 scale + reshape 为 [o_groups, o_lora_rank, dim]，推理时走 BF16 einsum 路径
...
|---|---|---|---|
| GPU VRAM (RTX 5060Ti 16G) | ~5.6 GB (扣除非专家+KV+激活) | **~439 个** | ~1.7 层 |
...
| 缓存级 | 可用容量 | 可容纳专家数 | 等效层数 |
|---|---|---|---|
| GPU VRAM (RTX 5060Ti 16G) | ~5.6 GB | **~233 个** | ~0.9 层 |
...
- **强烈推荐使用 FP4 模型**：单专家 12.75 MB，GPU 可缓存 ~439 专家（~1.7 层），CPU 可缓存 ~24.5 层，SSD 全路径延迟可接受
- **GPU 缓存有限但关键**：~439 专家虽仅覆盖 ~1.7 层，但 MoE 路由中高频专家跨层复用率高（如常见 token 模式），LFU 策略可有效提升命中率
...
struct GpuExpertCache {
    ...
    lfu: LfuCounter,
    capacity: usize,                        // ~439 个专家 (FP4) / ~233 个专家 (FP8)
...
槽位分配策略:
  - 总槽位数: ~439 (FP4) / ~233 (FP8)
  - 常驻槽位: LFU Top-K 热点专家，不参与淘汰（约 60% 槽位 = ~263 专家）
  - 动态槽位: 预取目标 + 当前层计算使用（约 40% 槽位 = ~176 专家）
  - 预取预留: 每层预留 6 个槽位给预取专家（176 / 6 ≈ 29 层预取窗口）
...
16. **专家权重 FP4 优先**：单专家 12.75 MB (FP4) vs 24 MB (FP8)，FP4 模型 GPU 可缓存 ~439 专家，SSD 全路径延迟 ~9.88 ms 可接受；FP8 模型 GPU 仅缓存 ~233 专家，SSD 全路径 ~18.6 ms 难以隐藏
17. **wo_a BF16 einsum**：convert.py 将 wo_a 从 FP8 反量化为 BF16 [8, 1024, 4096]，推理时走 cuBLAS batched GEMM einsum，不走 FP8 GEMM
